...
- (BOOL)insertObject:(id)data intoTable:(NSString *)tableName{
    
    NSString *colnamessql= [[NSString alloc]initWithFormat:@"pragma table_info([%@])",tableName];
    FMResultSet *result=[self executeQuery:colnamessql];
    NSMutableArray *columns=[NSMutableArray array];
    while ([result next]) {
        FMColumn *col=[[FMColumn alloc] init];
        col.name=[result stringForColumn:@"name"];
        col.type=[result stringForColumn:@"type"];
        [columns addObject:col];
    }
    [result close];
    NSString *propString=@"";
    NSString *wenhaoString=@"";
    NSMutableArray *values=[NSMutableArray array];
    NSMutableArray *props =[ObjectUtil getNotArrayProperty:[data class]];
    for (FMColumn *col in columns) {
        
            propString=[propString stringByAppendingString:[[[NSString alloc]initWithFormat:@"`%@`",col.name] stringByAppendingString:@","]];
            id value=nil;
            wenhaoString=[wenhaoString stringByAppendingString:@"?,"];
            for (NSDictionary *dic in props) {
                if ([[dic[@"name"] uppercaseString]isEqualToString:[col.name uppercaseString]]) {
                    value=[data valueForKey:dic[@"name"]];
    ...
    if (propString.length>0) {
        propString=[propString substringWithRange:NSMakeRange(0, propString.length-1)];
        wenhaoString=[wenhaoString substringWithRange:NSMakeRange(0, wenhaoString.length-1)];
    }
    NSString *sql= [[NSString alloc]initWithFormat:@"insert into `%@` (%@) values (%@)",tableName,propString,wenhaoString];
